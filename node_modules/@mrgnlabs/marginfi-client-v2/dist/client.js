...
class MarginfiClient {
    ...
    // Factories
    // --------------------------------------------------------------------------
    constructor(config, program, wallet, isReadOnly, group, banks, priceInfos, mintDatas, feedIdMap, addressLookupTables, preloadedBankAddresses, bankMetadataMap, bundleSimRpcEndpoint) {
        this.config = config;
    ...
    static async fetch(config, wallet, connection, clientOptions) {
        ...
        const idl = { ..._1.MARGINFI_IDL, address: config.programId.toBase58() };
        ...
        let addressLookupTableAddresses = _1.ADDRESS_LOOKUP_TABLE_FOR_GROUP[config.groupPk.toString()];
        ...
        return new MarginfiClient(config, program, wallet, readOnly, marginfiGroup, banks, priceInfos, tokenDatas, feedIdMap, addressLookupTables, preloadedBankAddresses, bankMetadataMap);
    }
    ...
    async createMarginfiAccount(opts, createOpts) {
        ...
        const bundleTipIx = (0, _1.makeBundleTipIx)(this.provider.publicKey);
        ...
        const tx = new web3_js_1.Transaction().add(bundleTipIx, ...ixs.instructions);
     ...
     */
    async createPermissionlessBank({ mint, bankConfig, group, admin, seed, priorityFee, opts, }) {
        const dbg = require("debug")("mfi:client");
        const keypair = seed ?? web3_js_1.Keypair.generate();
        const priorityFeeIx = priorityFee ? (0, utils_1.makePriorityFeeIx)(priorityFee) : [];
        ...
        const tx = new web3_js_1.Transaction().add(...priorityFeeIx, ...bankIxs.instructions);
        const sig = await this.processTransaction(tx, signers, opts);
     ...
     */
    async createMarginfiGroup(seed, additionalIxs, opts) {
        const dbg = require("debug")("mfi:client");
     ...
     */
    async processTransactions(transaction, signers, opts) {
        let signatures = [""];
        let versionedTransactions = transaction;
        ...
        try {
            ...
            if (signers)
                versionedTransactions[versionedTransactions.length - 1].sign(signers);
        }
        catch (error) {
        ...
        try {
            if (opts?.dryRun || this.isReadOnly) {
                const response = await (0, mrgn_common_1.simulateBundle)(this.bundleSimRpcEndpoint, versionedTransactions);
                ...
                if (!!this.wallet.signAllTransactions) {
                    ...
                    base58Txs = versionedTransactions.map((signedTx) => bytes_1.bs58.encode(signedTx.serialize()));
                }
                else {
                    for (let i = 0; i < versionedTransactions.length; i++) {
                        const signedTx = await this.wallet.signTransaction(versionedTransactions[i]);
                        const base58Tx = bytes_1.bs58.encode(signedTx.serialize());
                ...
                const response = await (0, mrgn_common_1.simulateBundle)(this.bundleSimRpcEndpoint, versionedTransactions).catch((error) => {
                    throw new web3_js_1.SendTransactionError({
                        action: "simulate",
                        signature: "",
                ...
                signatures = await this.sendTransactionAsBundle(base58Txs).catch(async () => await Promise.all(versionedTransactions.map(async (versionedTransaction) => {
            ...
            if (error?.logs?.length > 0) {
                ...
                if (parsedError) {
                    ...
                    throw new errors_1.ProcessTransactionError(parsedError.description, errors_1.ProcessTransactionErrorType.SimulationError, error.logs, parsedError.programId);
                }
            }
            console.log("fallthrough error", error);
            throw new errors_1.ProcessTransactionError(parsedError?.description ?? "Something went wrong", errors_1.ProcessTransactionErrorType.SimulationError, error.logs, parsedError.programId);
        }
    }
    async processTransaction(transaction, signers, opts) {
        ...
        try {
            if (opts?.dryRun || this.isReadOnly) {
                ...
                const base58Tx = bytes_1.bs58.encode(versionedTransaction.serialize());
                let mergedOpts = {
                    ...mrgn_common_1.DEFAULT_CONFIRM_OPTS,
                    commitment: connection.commitment ?? mrgn_common_1.DEFAULT_CONFIRM_OPTS.commitment,
                    preflightCommitment: connection.commitment ?? mrgn_common_1.DEFAULT_CONFIRM_OPTS.commitment,
                ...
                signature = (await this.sendTransactionAsBundle([base58Tx]).catch(async () => [
    ...
    async sendTransactionAsBundle(base58Txs) {
        try {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                ...
                if (getBundleStatusResult.error)
                    throw new Error(getBundleStatusResult.error.message);
    ...
    async simulateTransactions(transactions, accountsToInspect) {
        ...
        try {
            if (transactions.length === 1) {
                ...
                response = await (0, mrgn_common_1.simulateBundle)(this.bundleSimRpcEndpoint, versionedTransactions, accountsToInspect);
                if (response.value.err === null) {
                    return response.value.accounts?.map((a) => (a ? Buffer.from(a.data[0], "base64") : null)) ?? [];
                }
            ...
            if (error?.logs?.length > 0) {
                ...
                if (parsedError) {
                    console.log("Parsed:", parsedError);
                    throw new errors_1.ProcessTransactionError(parsedError.description, errors_1.ProcessTransactionErrorType.FallthroughError, error.logs, parsedError.programId);
                }
            }
            console.log("fallthrough error", error);
            throw new errors_1.ProcessTransactionError(parsedError?.description ?? "Something went wrong", errors_1.ProcessTransactionErrorType.FallthroughError, error.logs, parsedError.programId);
        }
        ...
        console.log("fallthrough error", error);
        throw new errors_1.ProcessTransactionError("Something went wrong", errors_1.ProcessTransactionErrorType.FallthroughError, error?.logs ?? []);
    }
}
